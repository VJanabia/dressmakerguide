...
3. Build command: `npm run build` — Output directory: `dist` (there is nothing to install; no dependencies).
4. Add the custom domain `dressmakerguide.com`, then add `www.dressmakerguide.com` as a second
   custom domain and create a **Redirect Rule**: `www.dressmakerguide.com/*` → `https://dressmakerguide.com/$1`, status **301**.
5. SSL/TLS → Edge Certificates → enable **Always Use HTTPS** (this is the http → https redirect).
6. Verify: `http://www.dressmakerguide.com/` and `http://dressmakerguide.com/` both land on
   `https://dressmakerguide.com/` in one hop.

Netlify equivalent: build `npm run build`, publish `dist`, plus a `_redirects` rule
`https://www.dressmakerguide.com/* https://dressmakerguide.com/:splat 301!`.

The build also emits a `_headers` file with cache and security headers (Cloudflare Pages and Netlify both read it).
